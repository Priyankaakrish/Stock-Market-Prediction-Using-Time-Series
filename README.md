...
Both diagrams are generated by `docs/make_diagrams.py`, so they stay in step
with the code rather than drifting into a stale draw.io export:

```bash
python docs/make_diagrams.py          # SVG + 2x PNG (needs cairosvg)
python docs/make_diagrams.py --no-png # SVG only
```

Yellow callouts mark where the built system deliberately departs from the
original design: the one-day feature shift, walk-forward evaluation instead of
a single split, skill-vs-benchmark instead of MAE alone, and the selection gate
that refuses to promote a model with no edge.

**Every box carries a status badge**, because a diagram that shows intent while
looking like a description of a running system is the same failure as reporting
MAE with no baseline — it looks like evidence and isn't.

| Badge | Meaning | Covers |
|---|---|---|
| `RUNS` | executed here, output verified | ingestion, features, backtest, all models, evaluation, gate, MLflow, FastAPI, Spark, monitoring, tests |
| `CODE ONLY` | implemented and tested, never deployed | Docker/nginx/compose, SageMaker training + inference + registry |
| `NOT BUILT` | described only, no code | EC2 instance, security group, EBS, IAM, TLS |

Nothing on AWS was ever provisioned: no S3 bucket, no EMR cluster, no EC2
instance, no SageMaker job. Spark ran locally on one core; the S3 backend is
exercised against `moto`'s mock; the SageMaker entrypoints ran against a
simulated `/opt/ml` tree.
...
│   ├── monitoring.py             # drift, data quality, live performance
│   ├── storage.py                # data lake: local + S3 backends
│   ├── selection.py              # gated best-model selection
│   ├── persistence.py            # serving bundles
│   ├── predict.py                # inference (shared by CLI and API)
│   └── plots.py
├── pipelines/
│   ├── ingest.py                 # fetch → merge → quality-gate → lake
...
├── .github/workflows/ci.yml      # tests · smoke · lint · diagram drift
├── docs/
│   ├── make_diagrams.py          # generates both diagrams
│   ├── architecture.svg          # system architecture
│   └── data_flow.svg             # 10-stage pipeline
├── spark/
│   ├── feature_pipeline.py       # groupBy(symbol).applyInPandas
│   └── benchmark.py              # pandas vs Spark, measured
├── sagemaker/
│   ├── train.py                  # /opt/ml training entrypoint
│   ├── inference.py              # model_fn / input_fn / predict_fn
│   ├── launch_training.py        # submit a training job
│   └── register_model.py         # registry, with the approval gate
├── deployment/
...
│   └── test_monitoring.py        # PSI calibration, drift, quality checks
...
| `GET` | `/monitor/report` | data quality + drift + live performance |
| `GET` | `/monitor/quality` | freshness, gaps, OHLC validity |
| `GET` | `/monitor/drift` | per-feature PSI and KS vs the training window |
| `GET` | `/monitor/performance` | realised skill vs the random walk |
| `GET` | `/monitor/predictions` | recent logged predictions |
