...
├── tests/test_pipeline.py           # 39 tests (leakage checks first)
│
...
- **The Spark path is written but not executed** in the build environment — no Spark runtime was available. Its `--validate-against` parity check is the mechanism for verifying it on a cluster.
